...
# Clean CSV: remove quotes and extra spaces
with open(dataset_path, 'r') as f:
    lines = f.readlines()
clean_lines = [line.strip().strip('"') for line in lines]
csv_string = '\n'.join(clean_lines)
...
# ========================================
# SIDEBAR FILTERS (Now includes Project!)
# ========================================
st.sidebar.header("Filters")
...
departments = st.sidebar.multiselect("Department", df["Department"].unique())
environments = st.sidebar.multiselect("Environment", df["Environment"].unique())
projects = st.sidebar.multiselect("Project", df["Project"].unique())  # NEW!
...
if environments:
    mask &= df["Environment"].isin(environments)
if projects:
    mask &= df["Project"].isin(projects)  # NEW!
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Data Exploration", "Cost Visibility", "Tagging Compliance",
    "Visualization Dashboard", "Tag Remediation"
])
...
with tab3:
    ...
    st.download_button(
        "Export Untagged to CSV",
        untagged_df.to_csv(index=False).encode(),
        "untagged.csv",
        "text/csv"
    )
...
with tab5:
    ...
    if st.button("Update and Recalculate"):
        ...
        edited_df['Tagged'] = edited_df[tag_columns].apply(
            lambda row: 'Yes' if all(row != 'Unknown') else 'No', axis=1
        )
        st.session_state.edited_df = edited_df
    ...
    st.markdown("### Reflection")
    st.write(
        "Improved tagging enhances accountability by attributing costs to specific departments and projects. "
        "It improves reporting accuracy and helps in better governance by identifying and reducing hidden costs."
    )
